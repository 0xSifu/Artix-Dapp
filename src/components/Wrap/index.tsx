...
function Wrap({ open, handleClose }: IAdvancedSettingsProps) {
    ...
    const waxBalance = useSelector<IReduxState, string>(state => {
        return state.account.balances && state.account.balances.wax;
    });
    ...
    const sartixAllowance = useSelector<IReduxState, number>(state => {
        return state.account.wrapping && state.account.wrapping.sartix;
    });
    ...
    const hasAllowance = useCallback(() => sartixAllowance > 0, [sartixAllowance]);

    const trimmedsartixBalance = trim(Number(sartixBalance), 6);
    const trimmedwaxBalance = trim(Number(waxBalance), 6);

    const getBalance = () => (isWrap ? `${trimmedsartixBalance} sARTIX` : `${trimmedwaxBalance} wAX`);
            ...
            <Paper className="ohm-card ohm-popover wrap-token-poper">
                <div className="cross-wrap wrap-cros-wrap">
                    ...
                    <div className="wrap-price" onClick={() => setIsWrapPrice(!isWrapPrice)}>
                        <p>
                            1 {isWrapPrice ? "sARTIX" : "wAX"} = {`${trim(wrapPrice, 4)} ${isWrapPrice ? "wAX" : "sARTIX"}`}
                        </p>
                    </div>
                </div>
