...
import { wAXTokenContract } from "../../abi";
...
export const changeApproval = createAsyncThunk("wrapping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const sartixContract = new ethers.Contract(addresses.SARTIX_ADDRESS, wAXTokenContract, signer);

    let approveTx;
    try {
        const gasPrice = await getGasPrice(provider);

        approveTx = await sartixContract.approve(addresses.WAX_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
    ...
    const waxAllowance = await sartixContract.allowance(address, addresses.WAX_ADDRESS);

    return dispatch(
        fetchAccountSuccess({
            wrapping: {
                wax: Number(waxAllowance),
            },
        }),
    );
});
...
export const changeWrap = createAsyncThunk("wrapping/changeWrap", async ({ isWrap, value, provider, networkID, address }: IChangeWrap, { dispatch }) => {
    ...
    const waxContract = new ethers.Contract(addresses.WAX_ADDRESS, wAXTokenContract, signer);
    ...
    try {
        ...
        if (isWrap) {
            wrapTx = await waxContract.wrap(amountInWei, { gasPrice });
        } else {
            wrapTx = await waxContract.unwrap(amountInWei, { gasPrice });
        }
...
const calcWrapValue = async ({ isWrap, value, provider, networkID }: IWrapDetails): Promise<number> => {
    ...
    if (isWrap) {
        const waxValue = await waxContract.SARTIXTowAX(amountInWei);
        wrapValue = waxValue / Math.pow(10, 18);
    } else {
        const sartixValue = await waxContract.SARTIXTowAX(amountInWei);
        wrapValue = sartixValue / Math.pow(10, 9);
    }
